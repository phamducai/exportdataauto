...
SELECT
  ...
  STr_SaleDtl.Goods_ID AS Goods_ID,
  Goods.Short_Name,
...
WHERE
  ...
  AND STr_SaleDtl.Goods_ID IN (166601, 166655, 224004, 224009, 224012, 224013, 224015, 224016, 224017, 224018, 240728) 
GROUP BY
...
# Định dạng tên file
csv_filename = f'Tien_Otsuka_{start_date_str}_to_{end_date_str}.csv'
