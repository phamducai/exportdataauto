...
WHERE
    a.Trans_No IN (
        ...
        WHERE StkTrCls_ID = 3033 
          AND Disabled = 0 
          AND EfTran_Date >= CONVERT(DATE, DATEADD(DAY, -6, GETDATE()))
          AND EfTran_Date < GETDATE()
    ) 
GROUP BY
    a.trans_no,
    EfTran_Date,
...
ole_date = datetime.now() - timedelta(days=6)
old_date = ole_date.strftime("%Y-%m-%d")
excel_filename = f'draft_{old_date}_to_{current_date}.csv'
