...
SELECT
    ...
    a.P_OrderID
FROM
    (SELECT DISTINCT tran_num, tran_date, id, PMT_code, amount, P_tran_date, P_tran_time, P_tran_ID, P_OrderID FROM ten_det WHERE PMT_code NOT IN ('CASH', 'DEPOS')) a
    INNER JOIN (
        SELECT
        ...
        FROM sle_trn
        WHERE
            ef_date BETWEEN DATE_FORMAT(CURDATE() - INTERVAL 1 MONTH, '%%Y-%%m-01') AND CURDATE()
        GROUP BY
            tran_date,
            ef_date,
            id,
...
# Tính toán ngày mồng 1 của tháng trước và ngày hiện tại
today = datetime.now()
