...
# Tính toán ngày mùng 1 của tháng trước và ngày mùng 1 của tháng này
today = datetime.now()
first_day_of_current_month = today.replace(day=1)
first_day_of_previous_month = (first_day_of_current_month - timedelta(days=1)).replace(day=16)
...
first_day_current_month_str = first_day_of_current_month.strftime('%Y-%m-%d')
	...
	CASE
		...
		FROM
			...
			WHERE
				...
				AND tran_date < '{first_day_current_month_str}'
			GROUP BY
				tran_date,
				ef_date,
				id,
